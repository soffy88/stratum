...
async def create_evidence(body: EvidenceCreate, user_id: str = Depends(jwt_auth)):
...
async def create_claim(body: ClaimCreate, user_id: str = Depends(jwt_auth)):
...
async def get_claim(claim_id: str, user_id: str = Depends(jwt_auth)):
...
async def update_claim(claim_id: str, body: ClaimUpdate, user_id: str = Depends(jwt_auth)):
...
async def create_relation(body: RelationCreate, user_id: str = Depends(jwt_auth)):
...
async def get_evidence(evidence_id: str, user_id: str = Depends(jwt_auth)):
    with get_conn() as conn:
        row = conn.execute(
            "SELECT id, source_highlight_id, substrate_id, quote, quote_hash, "
            "locator_json, confidence, created_at, updated_at "
