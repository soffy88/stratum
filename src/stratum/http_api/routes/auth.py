...
from ...middleware.abuse_detection import record_auth_event, _get_ip
import duckdb
import os
import hashlib
...
async def register(req: RegisterRequest, request: Request, db=Depends(get_db)):
    dao = UserDAO(db)
    if dao.get_user_by_email(req.email):
        raise HTTPException(400, "Email already registered")
    if dao.get_user_by_username(req.username):
    ...
    user = dao.create_user(
        email=req.email, username=req.username, password_hash=hash_password(req.password)
    )
    record_auth_event("register", _get_ip(request))
...
async def login(req: LoginRequest, response: Response, request: Request, db=Depends(get_db)):
    user_dao = UserDAO(db)
    user = user_dao.get_user_by_email(req.email_or_username) or user_dao.get_user_by_username(
        req.email_or_username
    )
    if not user or not verify_password(req.password, user.password_hash):
        record_auth_event("login_failed", _get_ip(request))
        raise HTTPException(401, "Invalid credentials")
    access_token = encode_access(user.email, user.corpus_id)
    token_str, refresh_hash = create_refresh(
        user.id, request.headers.get("user-agent"), request.client.host
    )
...
async def logout(request: Request, response: Response, db=Depends(get_db)):
    ...
    if token_str:
        ...
        db.execute(
            ...
            (datetime.now(timezone.utc), refresh_hash),
        )
