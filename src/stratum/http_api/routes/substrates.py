...
async def list_substrates(request: Request, medium: Optional[str] = None, limit: int = 50, db=Depends(get_db)):
    corpus_id = request.state.corpus_id
    subs = SubstrateDAO(db).list_substrates(corpus_id=corpus_id, medium=medium, limit=limit)
    items = [SubstrateItem(id=s.id, title=s.title, mime=s.mime, language=s.language,
                           page_count=s.page_count, created_at=s.created_at) for s in subs]
...
async def get_substrate(substrate_id: str, request: Request, db=Depends(get_db)):
    corpus_id = request.state.corpus_id
    s = SubstrateDAO(db).get_substrate(substrate_id=substrate_id, corpus_id=corpus_id)
    if not s:
        raise HTTPException(404, "Substrate not found")
    return SubstrateItem(id=s.id, title=s.title, mime=s.mime, language=s.language,
                         page_count=s.page_count, created_at=s.created_at)
...
async def get_derivatives(substrate_id: str, request: Request, db=Depends(get_db)):
    corpus_id = request.state.corpus_id
    # Verify substrate ownership
    s = SubstrateDAO(db).get_substrate(substrate_id=substrate_id, corpus_id=corpus_id)
    if not s:
        raise HTTPException(404, "Substrate not found")
    derivs = DerivativeDAO(db).list_by_substrate(substrate_id=substrate_id, corpus_id=corpus_id)
    return {"items": [DerivativeItem(id=d.id, kind=d.kind, seq=d.seq, content=d.content) for d in derivs]}
