...
> 〔Δ-note 版本 · r1 follow-up〕原文写 "3O v0.7"；**所有者 2026-07-22 裁：3O 现为 3.0**（v0.7 早已过时）。四命名空间实体（obase/oprim/oskill/omodul）不变，本 spec 一切 3O 引用按 **3.0** 读。详见附录 A.4。
...
| 3O 四命名空间（原文 v0.7 → 现 **3.0**） | 依据头 · §0.1 · §6 | **跨仓 hevi**：`docs/specs/3O-new-elements-manifest.md`（四命名空间 `obase/oprim/oskill/omodul`，范式 `obase←oprim←oskill←omodul←app`）+ `docs/specs/SPEC-oprim-new-primitives.md`；规范包在 helios-plat org（omodul PyPI） | ✅ 跨仓 · 版本已核（所有者 2026-07-22 裁 3.0，见 A.4） |
...
- **语义匹配一处（透明记，非 U、非悬空）**：① "无云端 API 依赖" 在 `STRATUM_SPEC_v0.2` 中以「本地优先」（§11.1 + L90）承载，无"无云端"字面；解析到实文、仅字面存差，据实报，不改挂原文。
- **版本已核（原悬项已闭）**：② "3O v0.7" —— **所有者 2026-07-22 裁：3O 现为 3.0，v0.7 早已过时**；四命名空间实体不变。裁决落地为依据头 Δ-note + A.1 状态，本 spec 3O 引用一律按 3.0 读。历史 verbatim 数字保留（原文若寻回以原文为准）。
