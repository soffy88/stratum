...
- **s1 曲沃代翼** → `ep:jin-decline-s1`：throughline=`thesis:shifu-modabizhe`（师服『末大必折』预言）；counterpoint=**显式无**（检索记录见 hevi QNLR-EP-jin-decline-s1 定稿；`thesis:shimo-changbian` 经 OP-D-057 否决嫁接，详下注）；beats=[封桓叔(:0205)→武公灭翼(:0217)]；证据链逐字白文 span；无 place 冲突。
  - **★拍数建议更新（补料 G1–G3，`CURATION-s1-supplement.md`，供 CC-A 重排、候选未终裁）**：封桓叔+师服宗法公理(:0205，G3 account 显式拆宗法半句可独立成旁白拍)→庄伯弑孝侯立鄂侯(cand:zhuangbo-shi-xiaohou，`…:0207`)→庄伯累次伐翼/晋人历次拥立(cand:zhuangbo-lei-fa-yi，`…:0083/:0086/:0097`，排序待裁)→武公诱杀小子侯(cand:wugong-you-sha-xiaozihou，`…:0262`)→桓王命緡立/武公退保曲沃(cand:huanwang-ming-min，`…:0270`)→武公灭翼逐缗(:0217，既有)→**新增**武公赂周釐王命曲沃伯为晋侯(G1 account `ac:shiji-wugong-huilu-mingzhu`，史记`:0020`，原无此拍——原拍数在"武公灭翼"即止，未覆盖"赂王受命"环节，遗漏一拍)。**counterpoint side-panel 候选**（非 thesis counter_refs，结构对照）：郑伯克段于鄢（`…:0004/:0012/:0013/:0014`，G3 发现，是否建 `thesis:zhengbo-ke-duan` 留裁）。
