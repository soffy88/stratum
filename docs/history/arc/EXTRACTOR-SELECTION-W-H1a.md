...
# W-H1a-3 更新 · ≥30 段 held-out（全文左传）+ 全自动可否结论

评测集 = **全文左传晋系 34 段**（held-out，与 gold 事件不重叠优先，gold 结构不入 prompt），qwen3-8b，date 不产出（D-028）。

| 指标 | 结果 |
|---|---|
| 有效 JSON 率 | **34/34（100%）** |
| event_type ∈ 枚举 | **34/34（100%）** |
| 注册表 person 解析率 | 6/118（**5%**） |
| 新人物候选 | 112 |
| 同一性判定（对 25 gold 粗比） | 0 匹配（采样为非 gold 新事件，符『非重叠优先』） |

**★全自动可否结论**：
- **结构可靠性达全自动线**：有效 JSON 与 type 枚举合规 **双 100%**，title/事件抽取质量高（如『荀息假道伐虢』『楚巴鄧之战』正确）。
- **但注册表解析率仅 5%**——非模型失败，而是**注册表过小**（现 ~40 person，皆 25 fixture 专属）；抽全书新事件必然涌出大量注册表未有的真人物（112 候选：姜氏/荀息/宋师…）。**person 解析率趋势**：随候选人物经审重入注册表，解析率将随注册表覆盖上升（本波 5% 为**冷启动基线**）。
- **裁决**：qwen3-8b **定型为半自动规模抽取器**（结构全自动可信）；**全自动仍不放行**——门槛 = ① 注册表经候选审重扩容至解析率稳定（建议 ≥70%）② 同一性判定接管（P2）后一致率验收。抽取候选（含新人物/地名 candidate）**永不直接成 gold**，随包送顾问审重（§4.4 红线守）。
