...
## D-023 · F1 撤 cf:jinyang-independence（折算记录移事件级）+ 契约 v0.2.2 + U7 关闭
...
- **红线遵守**：v0.2/v0.3 既有 tag 未动（新增 v0.2.2/v0.3.1，旧 tag 留痕）；未触碰 hevi 仓。
...
- **日期**：2026-07-22 · **决策人**：Wiki
- **裁决①**：`work_log` 之 `kind:agent` **就地裁定**——W-H1 本来就由 agent 执行，**agent 分钟就是排期的运营单位，无需换算人时**；人时只在 **Wiki 闸口动作**（亲裁/抽验/签署）单独记账。**换算率问题撤销**，各处『换算待裁』挂账句同步清除（本 commit）。
- **裁决②（附记）**：SPOTCHECK 引文『凭记忆备录、案头对原书』之不便＝**语料未入库的直接代价**——W-H1 落地（语料层入库、`para_ulid` 落位）后，**此类引文核对即机核项**（quote_span 对语料字节程序比对），出人工案头清单。
- **另**：九条裁决执行（`539b8d1..44d9cbd`）经 Wiki 对账**全部追认**；F2 账目（裁意→真签）裁定为彻底干净。
