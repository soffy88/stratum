...
- D-065（权威）**`identity_pipeline.py`"相邻事件共享交接角色"型假合记为已知限制 `IDENTITY-DEBT-1`，不为单例修改规则；同型累计 3 例触发规则修订评审。数据不得为消除假合而调整**。本条因 `ev:quwo-wugong-mie-yi`×`ev:zhuangbo-lei-fa-yi` 假合（弧灌注复核，见 D-064）而立。
- D-066（权威）**`ev:quwo-wugong-mie-yi` 题名与时间窗收窄至"武公伐翼虏哀侯（前709）"追认；终局另立 `ev:quwo-mie-min-liehou`（前678）；id 保留不重命名，注记沿革**。落地（回溯核查）：既往含旧题旨"灭翼·代晋"混叙句的 stratum 侧引用逐处加更正注记、原文字节保留——`arc/ADJUDICATION-s1-s2.md`（原裁决表行）、`arc/CURATION-s1-s2.md`（原候选表行+证据链行）、`arc/CURATION-s1-supplement.md`（D1 边界描述+文首）、`arc/ADJUDICATION-s1-supplement.md`（cf 拟建旧名→正式名对照）均已加注；`arc/EPISODEPLAN-MAPPING-DRAFT.md` 终态 8 拍序同步刷新，标 `READY-FOR-RELAY`。hevi 侧（N0 试点 s1 净稿，若确含同构混叙句）不在本仓核查/更正范围内（边界规矩，本仓不写 hevi）——待更正点列入 `EPISODEPLAN-MAPPING-DRAFT.md` §五清单，随本文档交付供 CC-A 在 hevi 侧自行核实处置。
- D-067（权威）**结构化字段值必须有原文字面依据；按在位区间、常识或推理填入的具体 id 一律不建，原文保留于 account**。本条因 `per:jin-ehou`（无字面依据推定，自查移除）与 `per:zhou-huanwang`（误填，四查抓出，原文实作周平王）而立。落地：周平王/隐公五年张力精化标注——"周平王卒于前720（传统纪年），而左传:0083 系隐公五年（前718），史记:0015 作『周平王使虢公』——二者不合；可能为追述旧称或文本讹误"，标待外部考订项，登 `seeds/chronology.json::resolutions[7]`（chronology override 候选位，`canonical`=TODO，不自行判定）。
- D-068（权威）**跨仓术语在指令中出现时须附来源仓与含义；执行方查无即报、不得推测对齐**。本条因『H4』指 hevi 侧 `R-hard` 第四门『名从注册表』（hevi 工作树内规格，本仓不持有）、stratum 查无此规格、CC-B 正确报『查无』（未推测对齐）而立。**追认**：`per:referent-ambiguous` 哨兵设计适配 H4，无需调整（本仓不可访问 hevi 自证，仅记录追认结果）。落地：`PERSON-INTAKE-PROTOCOL.md` 补注 H4 来源说明。
- D-069（权威）**身份不明的角色位以哨兵占位表达，不删除结构化条目（宁冗余不误删的字段层形态）；占位不得用于本可查实而未查者**，首例红线见 `PERSON-INTAKE-PROTOCOL.md`。本条追认 G4 收尾（`per:referent-ambiguous`）的设计原则并入决策日志。
- D-070（权威）**同名不同时的头衔持有者（如相隔 26 年的两处"虢公"）无字面依据不得合并，分别占位**——OP-D-069 的时间维推论。**追认** `registry-backfill-003` 全部处置：`ev:zhuangbo-lei-fa-yi`/`ev:quwo-mie-min-liehou` 两处"虢公"（左传 `:0086`/史记 `:0015` 与 `:0500`，相隔约26年）各自独立占位、未合并；`ev:zhuangbo-lei-fa-yi` 补入尹氏、武氏（左传 `:0083`，此前仅存于账户 prose、未入结构化 `actors`）；"翼侯"泛称覆盖复核（referent-ambiguous 占位 1 处 + 地名互证确证具体身份 1 处，无遗漏）。
- D-071（权威）**registry 覆盖模型由"主 actor 优先"改为"白文提及即入（具名建条目，不具名占位）"；策展包交付前须完成全 account 白文人名地名扫描**。本条因 s1 连续三批（G4 三轮）N0 报缺、次要角色反复回流而立。落地：s1 全 11 account 白文全量扫描（G4 终批，`registry-backfill-004`），新建 `per:qi-huangong`（齐桓公，史记:0020 系年参照点，非本事件参与者但具名即入）+ `pl:xingting`（陘庭，左传:0217『次于陘庭』，补入 `ev:quwo-wugong-mie-yi` 的 `geo.place_refs`）；其余全部已在/占位覆盖（详变更清单）。
- D-072（权威）**语料入库遇 wikisource 生僻字模板（`{{SKchar|N}}`/`{{SKchar2|N}}`）以 `【SKchar:N】`/`【SKchar2:N】` 占位保留，不臆断替代；占位不计入逐字机核失败**（原字形因底本扫描/模板缺失客观不可得，非抄录疏漏，非违反 R5 逐字纪律）。落地：`corpus/REGISTRY.md` R5 纪律段补注；`corpus/zuoshiboyi.json` 全部占位实例（401 处：387 SKchar + 14 SKchar2）遵此口径。
- D-073（权威）**开立 `arc:historiography-method`（讲史/治学方法论弧，跨代收纳、不限先秦）；两候选转正为 `thesis:zuqian-lihuandeshi`（取史事疏论断）、`thesis:zuqian-yangjiyipiao`（论断当公诸求正）**。本条因任务显式指名而立。落地：新建 `arc/arc-historiography-method.json`（复用 `arc-thesis.schema.json`，D-025 仓内对象，不进查询契约）；《左氏博议》正文25卷同轮全量入库（实测126篇，四库提要"168篇"系《通考》二十卷本篇目计法、非本 SKQS 二十五卷本切分，两版本口径不同，如实注记非误差强合）；正文按 OP-D-061 同构候选扫描法（不止关键词）另得4条新候选，本轮未转正、送后续裁决（详见 `arc/ADJUDICATION-methodology-arc-open.md`）。
- D-074（权威）**合规开放白文源穷尽检索仍查无的目标文本，登记 backlog（`corpus/BACKLOG-SOURCES.md`），收口不再重复检索，待合规源出现后复启**。本条因左宗棠论读史材料（《左文襄公全集·家書》）经 wikisource Author 页 + ctext 检索均查无（现存公开整理本不满足 R5 白文红线）而立，首例 `BACKLOG-SRC-1`。
- D-075（权威）**registry 回填前必须按姓名（`names_by_source`）全库检索，不能只查 id 列表——即使沿用不透名批次 id（如 `i4-XXX`）的既有条目也可能已覆盖同一人名，需交叉核对；`identity_pipeline.py` 的 person 异名归一检测（candidate-same 链）是此类重复的有效安全网，建新人物条目后须跑一次核对**。本条因 s2 弧灌注新建人物时 3 例（郤芮/重耳/荀息）与既有 `per:i4-006`/`per:i4-011`/`per:i4-012`（W-H1a-4-001 批次、Wiki 已准 candidate-same）重名而立——已修正：删除本轮重复条目，`arc/events/s2-liji.json` 全部引用改指既有 id（id 沿用不改，已被多处文档引用），补充既有条目的史记侧 attestation。
