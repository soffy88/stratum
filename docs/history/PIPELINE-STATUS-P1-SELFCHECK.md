...
| 2 | 在库源机核 100%（豁免清单显式） | ✅ **PASS** | **在库源 54/54 account 全 HIT（100%）**（三国志命名 bug 修正解锁 F3/F5/F6/F7/F8/F15/F17/F18）；**源级豁免显式**：4 竹书古本（辑佚无 canonical 白文）；2 PENDING-无开放白文（水经注/苏秦策，待 dump）。弧 5 论点全 HIT。清单 `corpus/SOURCE-MACHINE-CHECK-STATUS.md` |
...
**总判（自评，不自宣通过）**：**6/6 判据 PASS**（按 OP-D-036 重定域后）。**残留非 P1 门**：2 无开放白文 account（水经注/苏秦策）待 dump（OP-D-037 后台下载中）——属机核『PENDING-无源』、不阻塞 P1（在库源已 100%）；同一性一致率/全自动放行已移 P2。
