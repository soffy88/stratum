...
| DEBT-3 | **MINERU-AII-INTEGRATION-SPEC-001**（解析） | ~~W-H1 灌注前（硬需）~~ | U5 | ✅ **已销（2026-07-23，OP-D-049）**：MINERU spec 已入仓（合流 `4a68751`，原 verbatim `c39d987`）；conformance 附录 A.1（承自核验）+ A.3（D1–D5 delta 处置，D4=Track A 观察 2 周 PENDING 照挂、非伪造）+ §9（三轨白文适用性判定，D3——W-H1 白文路径已实跑：corpus 5487 段 wikisource 白文入库、走 §9.1 下半段，判定得实测背书）+ A.5 销账 均在册。 |
...
**U7 裁决（权威原文，W-H1a-5-P2 · 顾问 Claude · Wiki 授权 D-023 · FULL AUTO 块内下达）**：
> 『**互见/互补关系不设一等对象**；**多 account 挂同一事件即互见登记**，事件级文字表达转正式规范；**重开条件 = 出现带查询形状的实际消费者**。』
>
> **正式规范（本裁转正）**：一 event 挂 ≥2 account 且诸 account 系同源同说的叙事侧重差（非对立主张）→ 即『互见登记』，以 `mainline_decision.rationale`（拼合说明）+ account `genre_note`（侧重注记）事件级表达承载，**不铸 cf、不铸互见对象**。
> **曾用留痕**：D-023 版裁决文（措辞较长，理由三点）为本条前身，语义一致；本波以块内权威原话转正。
>
