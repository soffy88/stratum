...
| # | 问题 | 来源 | 现状 |
|---|---|---|---|
| U7 | **互见/互补关系是否需要一等对象（非 cf）**——F12 抽验裁『侧重差不立 cf』（D-022）后，互见互补仅存于 `mainline_decision` 拼合说明与 account `genre_note`，无可检索对象；若生产端/回归网需查询『互见关系』，须契约增对象（形状变更 = `contract_version` bump + 双端同步） | D-022（F12 修订牵出） | **候 Wiki 裁**；未裁前一律事件级文字表达，不私设新对象/新字段 |
