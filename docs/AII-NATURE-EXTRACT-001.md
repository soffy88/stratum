...
> **边界:** 纯 AII 本地——扩 onto_prompts.py + onto_persist + 接 converge_natures。**不动主库**(主库提供抽取动作，抽什么怎么抽是 AII 的事)。
>
> **路线选择(实证后定):**
> - ❌ 路C(改 oskill 收 dict 概念)：动主库 Owner SPEC，不走。
> - ❌ 路B(抽后 enrichment 单独跑 LLM 判)：多一遍调用、上下文割裂、贵。
> - ✅ **路A(骑 ku_candidate 字段)**：概念的 level/discipline/nature 挂在"定义该概念的 KU"上产出。
...
概念出现在很多 KU 里(价格弹性出现在 20 个 KU)，但**只有"定义/承载该概念的那条 KU"负责产出它的 level/discipline/nature**，避免多条 KU 对同一概念给出冲突判断。
...
一条 conceptual KU(如"价格弹性的定义") 额外产出:
  ...
  concept_nature:   (仅 abstract 且能抽到)   该抽象概念的本性
```

> 非 conceptual 的 KU(程序/解释/事实等)不产这些——它们引用概念，但不"定义"概念。
> 一个概念的 level/discipline/nature，由**定义它的那条 conceptual KU**一次性产出。
...
If the concept is ABSTRACT, attempt to extract its NATURE (本性):
  The nature is the concept's intrinsic LAW or NECESSARY TENDENCY —
  "how it must behave / where it must tend" — NOT "what it looks like" (that is mere appearance/相).
  Example: entropy's nature = "without external force, can only increase over time, irreversible,
  has a direction" — NOT "a measure of disorder" (that is its appearance).

  Ways to find nature (open, no upper limit):
  - Same word across domains: a word naming different things in different fields
    ...
    sensed a shared nature. Strip the domain shells, find the common intrinsic law.
  - Different words, same nature: you may directly recognize that differently-named concepts
  ...
  Output concept_nature if you can identify it; leave NULL if you genuinely cannot.
  ★Never fabricate a nature. NULL is correct when there isn't a clear intrinsic law.
```
...
### 3.1 写概念的 level / discipline / nature
...
对每条 conceptual KU，若它产了 defines_concept：
  ...
  → 写入 concept_level → concept_onto.level
         ...
         concept_nature → concept_onto.nature (若非空)
冲突处理：同一 canonical 概念被多条 KU 赋值时，
  - level/discipline：取首个非空；若冲突，记日志(不同KU判同概念学科不一致=信号，待观察)
  - nature：取首个非空(一个概念一个本性)
```

### 3.2 算 nature_vector(接通 converge_natures)

```
对有 nature 的概念：
  nature_vector = vector_encode(nature)   # 带 nature 类型(方案A:存 nature_vector 字段，与 concept vector 隔离)
  写入 concept_onto.nature_vector
→ 现成的 converge_natures 此时才有数据：
   扫描所有 nature_vector，≥2 个抽象概念的本性向量收敛(相似度≥阈值)
   → 凝结 nature_concept + 相关概念 nature_concept_id 指向它 + 本性同源连接
```

> 这接通了之前**空转的 converge_natures**——nature 有数据后，本性收敛才真正工作。
...
- **本性同源(converge_natures 凝结 nature_concept)需要跨学科**：要等物理书、信息论书等不同学科的书摄入，"熵"在多领域出现，本性才收敛涌现。

**所以本性维度的真正威力，要等知识库跨学科后才显现**(呼应"几万 KU、多学科后用不同方式找联系")。**现在做：prompt + 持久化 + 接通 converge_natures 的结构先建好、能抽就抽、结构充实；本性同源的涌现是后话。** 不因单本稀疏而不做——结构对了，跨学科书一进来，本性自动开始收敛。

---

## 五、落地步骤

1. 扩 PASS2_CHUNK_TMPL：conceptual KU 产 defines_concept/concept_level/concept_discipline/concept_nature(§2)。
2. onto_persist：读这些字段，写 concept_onto.level/discipline/nature(§3.1)。
3. 对有 nature 的概念算 nature_vector 写入，接通 converge_natures(§3.2)。
4. 验证(等 API 余额)：在 micro_clean 或一章上跑，看：
   ...
   - 抽到几个 nature(单本经济学预期稀疏，有几个即证明 prompt 工作)
   - converge_natures 是否空转(单本预期仍空，等跨学科)

---

## 六、命门

- 纯 AII 本地(扩 prompt + onto_persist + 接 converge_natures)，**不动主库**。
- 概念信息骑"定义它的 KU"，避开 oskill 的 str 过滤。
- 本性：道非相；途径开放；**抽不到留空，绝不造假**。
- 一个概念一个 level/discipline/nature(由定义它的 KU 产)，冲突取首个非空+记日志。
- nature_vector 用方案A(独立字段隔离)，不在向量内塞标记维(实测证明会毁区分度)。
