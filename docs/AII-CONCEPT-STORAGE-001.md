...
> **现状(实证):** concept_onto 是纯名字登记表(name 唯一/中文名/别名)，无向量、无 level/discipline/invariant；去重只到字符串同名(语义同概念碎成多条)；extractor 只产裸概念名；invariant 表不存在。
>
> **分两层落地:**
> - **存储层(AII 本地可做)**：扩 concept 表 + 建 invariant 表 + 概念向量化与语义归一 + 本性向量收敛。**先建好，结构在那能存。**
> - **抽取层(主库 oskill，Owner SPEC)**：让 extractor 产 level/discipline/invariant。**结构先到位，抽取跟上就填；未填的留空。**
...
| invariant | text \| null | 该抽象概念的本性(内在规律/必然趋势)；没有则 NULL |
| invariant_vector | vector \| null | 本性向量(带 invariant 标记维)；invariant 为空则 NULL |
| invariant_concept_id | uuid \| null | 若本性已凝结为本性概念，指向它 |

(保留原有 name/name_zh/aliases/created_at)

### 2.2 invariant_concept 表(新建，涌现产生)

| 列 | 类型 | 说明 |
|---|---|---|
| id | uuid PK | |
| statement | text | 这个本性是什么(如"无外力则只增不可逆") |
| vector | vector | 带 invariant 标记维 |
...
```
抽出/已有抽象概念的本性 → 算 invariant_vector(带 invariant 标记) →
  在已有 invariant_vector(及 invariant_concept.vector)里找最近：
    - 与另一个(或多个)抽象概念的本性向量收敛(相似度≥阈值) →
        凝结为 invariant_concept(若尚无)，相关抽象概念 invariant_concept_id 指向它
        → 这些抽象概念之间自动成立【本性同源】强联系
    - 暂无收敛对象 → 留作该抽象概念自己的 invariant(还不是本性概念)
```

> 同一个本性，无论从哪个概念、哪条途径、何时抽到，都向同一处收敛 → 共享即被发现，不靠逐对判断。**单个抽象概念的本性，在与别的收敛之前，只是它自己的 invariant，不进 invariant_concept 表。**
...
```
{
  ...
  invariant:      该抽象概念的本性(若是抽象概念且能抽到；否则空)
}
```
...
- 抽到则产 invariant，没有则留空——不硬编

> **此为 Owner SPEC，走主库流程。在它完成前，存储层先建好、extractor 暂只填 name，level/discipline/invariant 留空——结构在那，抽取跟上即填。**
> **过渡期可选**：AII 本地加一个"概念 enrichment 后处理"——抽完后用 LLM 对已存概念补判 level/discipline/invariant(不动主库)。是否做由后续决定。

---

## 四、落地顺序

1. **存储层(本地，先做)**：concept 扩 6 字段 + 建 invariant_concept 表 + 向量标记机制(先定方案A/B并实测) + 概念语义归一 + 本性收敛逻辑。
2. **在现有微观样本上验证**：把 micro_sample_body 的概念向量化、语义归一，看碎片化是否消除(price elasticity 系列是否归一)、根因 B 的"概念名接不到 KU"是否缓解。
3. **抽取层(主库，Owner SPEC)**：extractor 产 level/discipline/invariant。
...
- **本性没有就留空**——不硬编、不造假本性。
- **invariant_concept 只在 ≥2 抽象概念本性收敛时涌现**——非预设。
