...
export default function ConceptGraphPage() {
  ...
  const [colorMode, setColorMode] = useState<'discipline' | 'theme'>('discipline');
    ...
    <div className="aii-page-content flex flex-col gap-5 max-w-6xl mx-auto">
      ...
      <OGridFrame cols={{ sm: 1, md: 3 }} gap="sm">
        <label className="flex items-center gap-2 text-sm">
          <input
            ...
            onChange={(e) => setColorMode(e.target.checked ? 'theme' : 'discipline')}
          />
          按已固化主题染色(替代学科染色)
        </label>
        <label className="flex items-center gap-2 text-sm">
      ...
      <div className="rounded-lg border border-[color:var(--border)] bg-[color:var(--card)] p-2">
        <OKnowledgeGraph
          ...
          colorMode={colorMode}
          conceptTheme={themesState.data?.concept_theme}
          themes={themesState.data?.themes}
