...
#
# 用法:  nohup bash scripts/econ_flywheel_en_run.sh >> edu_pipeline/flywheel_edu.log 2>&1 &
...
mkdir -p .locks
exec 9>.locks/edu_flywheel.lock
flock -n 9 || { echo "⚠ 已有实例在运行({lock}), 本次启动退出"; exit 0; }

IDLE="${ECON_IDLE_SLEEP:-600}"
BOOKLIST="edu_pipeline/flywheel_edu_booklist.txt"
...
while true; do
    bash scripts/edu_flywheel.sh || echo "  ⚠️ 本轮飞轮异常(继续循环) $(date '+%H:%M')"
