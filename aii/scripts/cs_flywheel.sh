#!/usr/bin/env bash
# ★计算机/AI 飞轮(中考/高考/知识点/真题, 2026-08-10) — 自动循环: 发现书→检MD→反馈Stratum→跑管道→入库/隔离
...
FLYWHEEL_STATE="cs_pipeline/flywheel_cs_state.json"
FLYWHEEL_BOOK_LIST="cs_pipeline/flywheel_cs_booklist.txt"
FLYWHEEL_REPORT="cs_pipeline/flywheel_cs_report.json"
FLYWHEEL_LOG="cs_pipeline/flywheel_cs.log"
...
export ECON_BATCH_REPORT="cs_pipeline/batch_report.json"
export ECON_QUAL_DIR="cs_pipeline/qual"
export ECON_CKPT_DIR="cs_pipeline/ckpts"
...
mkdir -p cs_pipeline cs_pipeline/qual cs_pipeline/ckpts
...
# 合并 batch_report
batch = Path('cs_pipeline/batch_report.json')
if batch.exists():
