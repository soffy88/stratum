...
def norm(t):
    ...
    t = re.sub(r"\s+", " ", t).strip()  # 保留中文
    return re.sub(r"s\b", "", t)  # 单复数归一


# ★防御性过滤: [1/5]_plan 偶尔把"论断"当KU起名(该叫"经理人薪酬"却写成整句
# "经理人薪酬是对努力和风险承担的回报"), 这种句子级title不该被当"概念"收进概念图
# (实测 econ_zh_2726f38224 命中4条, 全库9条——规模小但会污染概念共现/KC摘要).
_SENT_EN = re.compile(r"\b(?:is|are|was|were|should|would|will|does|do)\b", re.I)
_SENT_ZH = re.compile(r"因为|因此|所以|导致|使得|表明|说明|意味着")


def _is_sentence(title: str) -> bool:
    if re.search(r"[一-鿿]", title):
        return len(title) > 20 and bool(_SENT_ZH.search(title))
    return len(title.split()) > 8 and bool(_SENT_EN.search(title))
...
async def go():
    ...
    kus = await c.fetch(
        f"SELECT ku_id,title,natural_text,(provenance->>'chapter')::int ch FROM aii.ku_onto WHERE substrate_id='{SUB}'"
    )
    # 1. 概念词表 = 去重归一标题(排除句子级title); canonical = 首次出现的清洗标题
    canon = {}
    for k in kus:
        if _is_sentence(k["title"]):
            continue
        n = norm(k["title"])
        if n and n not in canon:
            canon[n] = re.sub(r"\s*\([^)]*\)", "", k["title"]).strip()
    ...
    phrases = [
        (n, re.compile(r"(?<![a-z])" + re.escape(n) + r"(?:e?s)?(?![a-z])", re.I))
        for n in canon
        if len(n) >= 6 or (re.search(r"[一-鿿]", n) and len(n) >= 3)
    ]
    ku_concepts = {}
    for k in kus:
        body = (k["natural_text"] or "").lower()
        own = norm(k["title"])
        inv = {own} if own in canon else set()
        for n, rx in phrases:
            if n != own and rx.search(body):
                inv.add(n)
    ...
    for n, name in canon.items():
        row = await c.fetchrow(
            ...
            "ON CONFLICT(name) DO UPDATE SET name=EXCLUDED.name RETURNING concept_id",
            name,
            SUB,
        )
    ...
    for kid, inv in ku_concepts.items():
        for n in inv:
            if n in cid:
                await c.execute(
                    "INSERT INTO aii.ku_concept_onto(ku_id,concept_id) VALUES($1,$2) ON CONFLICT DO NOTHING",
                    kid,
                    cid[n],
                )
                links += 1
    ...
    if os.getenv("AII_COOCCUR", "0") == "1":
        ...
        await c.execute(
          ...
          GROUP BY a.ku_id,b.ku_id,ka.embedding,kb.embedding""",
            SUB,
        )
