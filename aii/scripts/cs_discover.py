"""★计算机/AI 飞轮(编程/算法/机器学习, 2026-08-10 夸克 254 本)(中考/高考/知识点/真题/考点, 2026-08-10 夸克最大类型 1985 本) — 发现未处理的书(任意学科,有章节结构).
扫 /home/soffy/books/MD/计算机/ → ≥3 章(第N章 或 # Chapter N)→
排除已入库(ingested_substrate 已登记, 不限KU数)→ 排除 flywheel 终态 → 输出书单.

收录经济/数学之外、有章节结构的教材(心理/哲学/科普等)。
substrate_id: 中文→cs_zh_<hash>,英文→cs_en_<hash>。
...
MISC_DIR = "/home/soffy/books/MD/计算机"
...
def _sid(stem: str, text: str) -> str:
    pref = "cs_zh" if _is_chinese(text) else "cs_en"
...
async def main():
    ...
    ap.add_argument("--state", default=str(ROOT / "misc_pipeline" / "flywheel_cs_state.json"))
