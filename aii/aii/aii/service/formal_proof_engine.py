    ...
    async def verify(self, ku: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if verified_data and verified_data.get("verified_count") == 1:
                ...
                logger.info(
                    f"Theorem '{name}' verified as proven via cached authentic metadata. Updating grade."
                )
                ...
                # "proven" is not a member of onto_vocab.VALID_GRADES (ku_onto's
                # grade CHECK enum) — a formally-verified theorem maps to "verified",
                # with grounded_by.method set so ck_ku_onto_grade_mandate is satisfied.
                await self.backend.record_state_change(
                    ku_id=str(ku["ku_id"]),
                    to_grade="verified",
                    reason="established_proof",
                    decision_trail={"evidence": evidence, "trail": decision_trail},
                    grounded_by={"method": "formal_proof", "evidence": evidence},
                )
