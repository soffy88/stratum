...
def convert_document(src: Path, kb_dir: Path) -> ConvertResult:
    ...
    # ★stratum 内化(2026-08-11): MD 大文件(>150KB)也走 long/PageIndex 分块 —
    #   教材/论文集 MD 常超 LLM 上下文, 原逻辑只对 PDF 判 long, 长 MD 会 short 编译上下文超限。
    if src.suffix.lower() in (".md", ".markdown"):
        try:
            if src.stat().st_size >= 150 * 1024:
                logger.info("Long MD detected (%d KB >= 150KB): %s", src.stat().st_size // 1024, src.name)
                return ConvertResult(raw_path=raw_dest, is_long_doc=True, file_hash=file_hash)
        except OSError:
            pass
